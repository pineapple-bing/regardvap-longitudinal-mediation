...
run_prespecified_sensitivities <- function(analysis_df, nsim = 4000, seed = 20260903) {
  specifications <- list(
    ...
    list(id = "S3_microbiology_information", description = "Exploratory model including daily microbiology-information proxy", mediator_history = "lagged", lt_variant = "main", include_ot = TRUE, horizon = 3L),
    list(id = "S4_early_treatment_window", description = "Early treatment window through Day 1 only", mediator_history = "lagged", lt_variant = "main", include_ot = FALSE, horizon = 1L)
  )
